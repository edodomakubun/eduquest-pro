...
export default function GenerateKisiPage() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        if (access === 'admin') {
          ...
          if(!formData.teacherName) setFormData(prev => ({...prev, teacherName: currentUser.displayName || ''}));
        } else {
          await signOut(auth);
          router.push('/login');
        }
  ...
  const generateKisiKisi = async () => {
    ...
    if (!isPremium && totalSoal > 10) {
      return showError('Akun Free maksimal menghasilkan 10 soal kisi-kisi. Silakan Upgrade Pro.');
    }
    ...
    try {
      ...
      } catch (e) { console.error("Gagal menyimpan riwayat kisi-kisi:", e); }
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2 text-indigo-600"><LayoutGrid className="w-6 h-6" /><span className="text-xl font-bold tracking-tight hidden sm:block">Buat Kisi-Kisi Ujian</span></div>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        {!isPremium && appState === 'FORM' && (
          <div className="bg-gradient-to-r from-amber-100 to-orange-50 border border-amber-200 p-4 rounded-2xl mb-8 flex justify-between shadow-sm animate-in fade-in">
            <div className="flex items-center"><ShieldAlert className="w-8 h-8 text-amber-600 mr-3" /><div><p className="font-bold text-amber-900">Versi Free</p><p className="text-amber-700 text-sm">Akun Free dibatasi maksimal 10 soal untuk pembuatan kisi-kisi. Upgrade Pro untuk tanpa batas!</p></div></div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={formData.grade} onChange={(e) => setFormData({...formData, grade: e.target.value})} className="w-full border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
                        <option>1 (Fase A)</option><option>2 (Fase A)</option><option>3 (Fase B)</option><option>4 (Fase B)</option><option>5 (Fase C)</option><option>6 (Fase C)</option>
                      </select>
                    </div>
                    <div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col h-full">
                ...
                <div className="space-y-4 flex-grow flex flex-col">
                  <div className="flex-1 flex flex-col">
                    ...
                    <p className="text-xs text-slate-500 mb-2">Tempelkan teks CP atau KD yang menjadi tujuan utama ujian ini.</p>
                    <textarea value={formData.cpText} onChange={(e) => setFormData({...formData, cpText: e.target.value})} className="w-full flex-grow min-h-[120px] p-4 border rounded-xl bg-slate-50 outline-none focus:ring-2 focus:ring-indigo-500 text-sm leading-relaxed" placeholder="Contoh: Peserta didik dapat mengenali, mengidentifikasi, dan mendeskripsikan ciri-ciri tumbuhan di lingkungan sekitar..."></textarea>
                  </div>

                  <div className="flex-1 flex flex-col pt-4">
                    <label className="block text-sm font-bold text-slate-700 mb-1">Lingkup Materi Pokok</label>
                    <p className="text-xs text-slate-500 mb-2">Masukkan ringkasan materi atau topik-topik yang akan diujikan.</p>
                    <textarea value={formData.materiText} onChange={(e) => setFormData({...formData, materiText: e.target.value})} className="w-full flex-grow min-h-[150px] p-4 border rounded-xl bg-slate-50 outline-none focus:ring-2 focus:ring-indigo-500 text-sm leading-relaxed" placeholder="Contoh: 1. Bagian-bagian tubuh tumbuhan. 2. Fungsi akar, batang, daun, bunga. 3. Fotosintesis dasar..."></textarea>
                  </div>
                </div>
                ...
                <div className="mt-6 flex justify-between items-center bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                  <div className="flex items-center text-sm font-medium text-indigo-800">
                    <Wand2 className="w-5 h-5 mr-2" /> AI akan meracik Indikator & Level Kognitif
                  </div>
        ...
        {appState === 'LOADING' && (
          <div className="py-24 flex flex-col items-center justify-center text-center">
            <div className="relative mb-6">
              ...
              <div className="absolute inset-0 flex items-center justify-center"><LayoutGrid className="w-8 h-8 text-indigo-600 animate-pulse" /></div>
            </div>
